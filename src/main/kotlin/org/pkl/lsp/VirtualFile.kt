...
import org.pkl.core.runtime.AxiomBridge
import org.pkl.core.runtime.AxiomTranspiler
...
enum class Origin {
  ...
  JAR,
  MODULEPATH,
  AXIOM_PYTHON; // Added for Axiom

  companion object {
    fun fromString(origin: String): Origin? {
      return try {
        // Handle "axiom-python" -> "AXIOM_PYTHON"
        valueOf(origin.uppercase().replace("-", "_"))
...
/** Represents a virtual Pkl module generated by introspection of a Python backend. */
class AxiomFile(override val uri: URI, override val project: Project) : BaseFile() {
  // We give it a generic name, or could derive it from the URI
  override val name: String = "axiom_generated.pkl"
  override val pklAuthority: String = "axiom-python"
  ...
  override fun doReadContents(): String {
    return try {
      logger.log("AxiomBridge: resolving $uri")
      // 1. Invoke the bridge (calls Python extractor -> gets JSON IR)
      val jsonIR = AxiomBridge.resolveFastApi(uri)
      // 2. Transpile the JSON IR into valid Pkl source code
      AxiomTranspiler.generatePkl(jsonIR, uri.schemeSpecificPart)
    } catch (e: Exception) {
      logger.error("Failed to bridge Python: ${e.message}")
      // Return a valid Pkl comment with the error so the editor doesn't crash completely,
      // but maybe users see the error via logs or empty module.
      """
      /// ERROR: Failed to introspect Python backend.
      /// ${e.message?.replace("\n", "\n/// ")}
      module axiom_error
      """
        .trimIndent()
